...
// 使用注解方式定义Servlet，用于处理"/upload_teacherImg"路径的请求
...
public class upload_teacherImg extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // 将文件保存到指定位置，并重命名为教师ID.jpeg
            smartFile.saveAs("/userImg/" + id + ".jpeg");

            // 上传成功，弹出成功提示，并重定向到教师的个人页面
...
/**类上的@WebServlet注解：
 @WebServlet("/upload_teacherImg")：指明该Servlet的访问路径是/upload_teacherImg。
 doPost方法：
 protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException：
 在当前例子中，将POST请求转发到doGet方法，确保GET和POST请求的处理逻辑一致。
 doGet方法：
 protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException：
 该方法处理HTTP GET请求，主要功能是处理文件上传请求。
 响应和请求的编码设置：
  ...
  **String id = rq.getParameter("id");**：从请求中获取教师ID。
  **File smartFile = smartUpload.getFiles().getFile(0);**：获取上传的文件对象。
  **smartFile.saveAs("/userImg/" + id + ".jpeg");**：将文件保存到指定目录，并重命名为教师ID.jpeg。
  **out.print("<script>alert("上传成功!");window.location.href='teacher/personal.jsp';</script>");**：如果上传成功，弹出提示信息并重定向到教师的个人页面。
 catch (Exception e) 块：
  **out.print(e);**：如果出现异常，打印异常信息到客户端。
 这个Servlet处理文件上传请求，并在成功上传后将用户重定向到教师的个人页面。*/